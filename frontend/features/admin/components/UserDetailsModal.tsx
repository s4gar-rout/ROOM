  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 animate-in fade-in duration-200">
      <div className="relative w-full max-w-md">
        <div className="w-full border border-[#174D35]/12 bg-[#FFFDF8] p-6 shadow-xl animate-in zoom-in-95 duration-200">
          
          {/* Header */}
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-serif font-medium text-[#1C1B18]">User Details</h2>
            <button onClick={onClose} className="text-[#5F554A] hover:text-[#1C1B18] transition-colors relative z-10">
              <X size={20} />
            </button>
          </div>
          ...
          <div className="flex flex-col items-center justify-center mb-8">
            ...
            <div className="flex items-center gap-2 mt-2">
              <span className="inline-flex bg-gray-100 px-2.5 py-0.5 text-xs font-semibold uppercase tracking-wider text-gray-800 border border-gray-200">
                {user.role}
              </span>
              {user.isBlocked ? (
                <span className="inline-flex items-center gap-1.5 bg-red-100 px-2.5 py-0.5 text-xs font-medium text-red-700 border border-red-200">
                  <Ban size={12} /> Blocked
                </span>
              ) : (
                <span className="inline-flex items-center gap-1.5 bg-green-100 px-2.5 py-0.5 text-xs font-medium text-green-700 border border-green-200">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3 p-3 bg-white border border-[#1C1B18]/10">
